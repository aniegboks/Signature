  ...
  return (
      ...
      >
        <div className="backdrop-blur-md bg-white/30 px-4 py-6 md:py-4">
          <div className="max-w-5xl mx-auto flex items-center justify-between rounded-full py-4 md:px-8 md:shadow-[0_4px_6px_-1px_rgba(0,0,0,0.05)]">
            {/* Left Nav */}
            <ul className="hidden md:flex items-center gap-4">
              {settings.data.navigation.map(({ link, label }) => (
                <li key={label}>
                  <PrismicNextLink
                    field={link}
                    className="text-[10px] sm:text-sm font-medium text-gray-600 hover:text-black transition font-body"
                  >
                    {label}
                  </PrismicNextLink>
                </li>
              ...
              <motion.ul className="flex flex-col gap-6">
                {settings.data.navigation.map(({ link, label }) => (
                  ...
                  >
                    <PrismicNextLink
                      field={link}
                      className="text-[10px] sm:text-sm font-medium text-neutral-600 hover:text-black transition"
                    >
                      {label}
                    </PrismicNextLink>
                  </motion.li>
